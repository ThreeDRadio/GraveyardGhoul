...
class MusicLibrary:
    ...
    def getRandomDemo(self):
        ...
        cur.execute("SELECT * FROM cd WHERE demo = '2' AND ghoul_approved = true ORDER BY RANDOM() LIMIT 1")
        cd = cur.fetchone()
    ...
    def getRandomLocal(self):
        cur = self.db.cursor(cursor_factory=psycopg2.extras.DictCursor)
        cur.execute("SELECT * FROM cd WHERE local = '2' AND ghoul_approved = true ORDER BY RANDOM() LIMIT 1")
        cd = cur.fetchone()
        cur.execute("SELECT * from cdtrack WHERE cdid = %s AND tracklength <= %s ORDER BY RANDOM() LIMIT 1;", (cd['id'], self.maxLength))
        track = cur.fetchone()
        return Song(cd, track)
    ...
    def getRandomAustralian(self): 
        cur = self.db.cursor(cursor_factory=psycopg2.extras.DictCursor)
        cur.execute("SELECT * FROM cd WHERE cpa = ANY(%s) AND ghoul_approved = true ORDER BY RANDOM() LIMIT 1", (self.ausNames,))
        cd = cur.fetchone()
        cur.execute("SELECT * from cdtrack WHERE cdid = %s AND tracklength <= %s ORDER BY RANDOM() LIMIT 1;", (cd['id'], self.maxLength))
        track = cur.fetchone()
        return Song(cd, track)
    ...
    def getRandomSong(self, female): 
        cur = self.db.cursor(cursor_factory=psycopg2.extras.DictCursor)
        if female == True:
            cur.execute("SELECT * FROM cd WHERE female = '2' AND ghoul_approved = true ORDER BY RANDOM() LIMIT 1")
        else:
            cur.execute("SELECT * FROM cd WHERE ghoul_approved = true ORDER BY RANDOM() LIMIT 1")
